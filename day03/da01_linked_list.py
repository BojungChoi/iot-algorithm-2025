...
dataArray = ['다현', '정현', '쯔위', '사나', '지효' ] # 연결리스트를 만들 데이터
...
def printNodes(start):
    ...
    while curr.getLink() != None: # 현재 링크에 다음링크가 있는동안 
...
def insertNode(findData, insertData):
    global memory, head, prev, curr # 전역변수 가져와서 inserNode 함수 안에서 쓰겠다는 선언.
    ...
    if head.getData() == findData:
        ...
        return # 더이상 밑으로 실행 안되도록 함수를 빠져나감 (밑에 출력값 때문에 화사가 두번씩나옴)
...
if __name__ == '__main__': # 시작모듈일때
    ...
    for name in dataArray[1:]: # 1번인덱스 '정연' 부터 사용
    ...
    insertNode('재남', '문별') # 재남을 검색해서 그자리에 문별을넣어라 -> 재남을 찾을 수 없으므로 순차적으로 찾다가 마지막에 문별이 삽입
    printNodes(head)

    # 데이터 검색
    fNode = findNode('쯔위') # 찾게되면 주소값이 나오므로 class str 함수를 만들어서 문자열 리스트로 출력 
    print(fNode)
    fNode = findNode('보정') # 찾을 데이터가 없다면 None 표시
    print(fNode)
